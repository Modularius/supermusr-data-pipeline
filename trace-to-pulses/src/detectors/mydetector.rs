...
impl Detector for LocalExtremumDetector {
    ...
    fn signal(&mut self, time: Real, value: Stats) -> Option<LocalExtremumEvent> {
        if let Some((prev_value,Some(prev_prev_value))) = self.prev {
            let return_value = {
                if (prev_prev_value < prev_value && prev_value >= value.mean) || (prev_prev_value <= prev_value && prev_value > value.mean) {
                    Some(
                        LocalExtremumData::with_value(LocalExtremumClass::LocalMax,prev_value)
                        .make_event(time - 1.)
                    )
                } else if (prev_prev_value > prev_value && prev_value <= value.mean) || (prev_prev_value >= prev_value && prev_value < value.mean) {
                    Some(
            ...
            self.prev = Some((value.mean,Some(prev_value)));
            return_value       
            ...
            self.prev = Some((value.mean,None));
            None
        }
    }
}
