...
impl ODEData {
    ...
    pub fn value(&self, time : Real) -> Real {
        Real::exp(self.lambda*time)*(self.coef_cos*Real::cos(self.theta*time) + self.coef_sin*Real::sin(self.theta*time)) + self.coef_const
    }
    pub fn deriv1(&self, time : Real) -> Real {
        Real::exp(self.lambda*time)*(
            (self.lambda*self.coef_cos + self.theta*self.coef_sin)*Real::cos(self.theta*time)
             + (self.lambda*self.coef_sin - self.theta*self.coef_cos)*Real::sin(self.theta*time)
        )
    }
    pub fn deriv2(&self, time : Real) -> Real {
        Real::exp(self.lambda*time)*(
            ((self.lambda*self.lambda - self.theta*self.theta)*self.coef_cos + 2.0*self.lambda*self.theta*self.coef_sin)*Real::cos(self.theta*time)
             + ((self.lambda*self.lambda - self.theta*self.theta)*self.coef_sin + 2.0*self.lambda*self.theta*self.coef_cos)*Real::sin(self.theta*time)
        )
    }
}
...
impl Display for ODEData {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_fmt(format_args!("{0},{1},{2},{3},{4},{5},{6},{7}",
            self.start, self.end,
            self.residual,
            self.lambda, self.theta,
            self.coef_cos, self.coef_sin, self.coef_const,
        ))
    }
}
        ...
        match result {
            Ok(((lambda,theta), coefs, residual)) => {
                let data = ODEData {
                    ...
                    residual,
                    lambda,theta,
                    coef_cos: coefs.0, coef_sin: coefs.1, coef_const: coefs.2,
                    ..Default::default()
                };
...
impl FeedbackDetector for MuonDetector {
    ...
    fn modify_parameter(&mut self, time: Self::TimeType, param : &FeedbackParameter<Self::ValueType>) {
        loop {
            if let Some(pulse) = self.recent_muons.front() {
        ...
        let val = self.recent_muons.iter().map(|pulse|pulse.value(time + 1.)).sum::<Real>();
        let deriv1 = self.recent_muons.iter().map(|pulse|pulse.deriv1(time + 1.)).sum::<Real>();
        let deriv2 = self.recent_muons.iter().map(|pulse|pulse.deriv2(time + 1.)).sum::<Real>();
