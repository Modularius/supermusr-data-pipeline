...
impl MuonData {
    pub fn new(peak : (Real,Real), max_slope : (Real,Real), min_slope : (Real,Real), duration : Real) -> Self {
...
impl Display for MuonData {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_fmt(format_args!("{0},{1},{2},{3},{4},{5},{6}",
            self.peak.0,self.peak.1, self.max_slope.0,self.max_slope.1, self.min_slope.0,self.min_slope.1,self.duration))
    }
}
