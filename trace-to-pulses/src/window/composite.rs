...
pub struct DoublingWindow<T,V> where
    T : Temporal,
...
impl<T,V> DoublingWindow<T,V> where
    T : Temporal,
    V : Default + Copy + Display + Debug,
{
    pub fn new() -> Self {
...
impl<T,V> Window for DoublingWindow<T,V> where
    T : Temporal,
    V : Default + Copy + Display + Debug,
{
    type TimeType = T;
    ...
    fn push(&mut self, value: Self::InputType) -> bool {
        self.pair = TracePair(value,value); true
    }
...
#[derive(Default, Clone, Debug)]
pub struct CompositeWindow<W1,W2,T> where
    T : Temporal,
...
impl<W1,W2,T> CompositeWindow<W1,W2,T> where
    T : Temporal,
    W1 : Window<TimeType = T>,
    W2 : Window<TimeType = T>
{
...
impl<W1,W2,T> Window for CompositeWindow<W1,W2,T> where
    T : Temporal,
    W1 : Window<TimeType = T>,
    W2 : Window<TimeType = T>,
