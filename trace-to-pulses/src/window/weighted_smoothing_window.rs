...
pub struct WeightedSmoothingWindow {
    value: Real,
    sum: Real,
    sum_of_squares: Real,
    size: Real,
...
impl WeightedSmoothingWindow {
    pub fn new(coefficients: &[Real]) -> Self {
        if coefficients.len() < 2 {
            panic!("Size must be >= 2");
        }
        WeightedSmoothingWindow {
            window: VecDeque::<Real>::with_capacity(coefficients.len()),
            size: coefficients.len() as Real,
            size_m1: coefficients.len() as Real - 1.,
...
impl Window for WeightedSmoothingWindow {
    type TimeType = Real;
    type InputType = Real;
    ...
    fn stats(&self) -> Option<Stats> {
        if self.is_full() {
            Some(Stats {
                value: self.value,
                mean: self.window.iter().zip(self.coefficients.iter()).map(|(b,c)|b*c).sum(),
...
mod tests {
    ...
    fn test_window_size_zero() {
        ...
        data.iter()
            ...
            .window(WeightedSmoothingWindow::new(&vec![]));
    }
    ...
    fn test_window_size_one() {
        ...
        data.iter()
            .enumerate()
            .map(processing::make_enumerate_real)
            .window(WeightedSmoothingWindow::new(&vec![1.]));
    }
    ...
    fn test_window_size_two() {
        ...
        assert!(data
            ...
            .enumerate()
            .map(processing::make_enumerate_real)
            .window(WeightedSmoothingWindow::new(&vec![1.,1.]))
            .next()
            .is_some());
    }
    ...
    fn test_no_data() {
        ...
        assert!(data
            ...
            .enumerate()
            .map(processing::make_enumerate_real)
            .window(WeightedSmoothingWindow::new(&vec![1.,2.,1.]))
            .next()
            .is_none());
    }
    #[test]
    ...
    fn test_variance_accuracy() {
        ...
        for window_size in 2..100 {
            let coefs = (0..window_size).map(|_|1.).collect_vec();
            let smoothing_window = WeightedSmoothingWindow::new(coefs.as_slice());
            let mut itr = data
                .iter()
                .enumerate()
                .map(processing::make_enumerate_real)
