...
pub(crate) mod tags {
    ...
    pub(crate) trait Tag<T: H5Type, P, O>: Clone where T: H5Type {
...
pub(crate) trait Buildable<T> where T: H5Type + Clone {
    type BuilderType;
...
pub(crate) trait Examine<R, C> {
    fn examine<F, T>(&self, f: F) -> T where F: Fn(&C) -> T;

    fn examine_children<F, T>(&self, f: F) -> T where F: Fn(&[R]) -> T;
}
