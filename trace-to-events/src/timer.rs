...
impl DescStats {
    pub(crate) fn print(&self) {
        ...
        println!(
            ...
            "Mean", "StdDev", "Skewness", "Median", "IQ Range", "Min", "Max"
        );
        println!(
            "|{0:10.3}|{1:10.3}|{2:10.3}|{3:10.3}|{4:10.3}|{5:10.3}|{6:10.3}|",
            self.mean, self.stddev, self.skewness, self.median, self.iq_range, self.min, self.max
        );
    }
}
...
fn moment<I: Iterator>(m: usize, num: usize, mean: f64, iter: I) -> f64
where
    I: Iterator<Item = u128>,
{
    iter.map(|x| f64::powi(x as f64 - mean, m as i32))
        .sum::<f64>()
        / num as f64
}
fn calculate_stats_from<I>(num: usize, iter: I) -> DescStats
where
    I: Iterator<Item = u128> + Clone,
{
    ...
    let (median, iq_range) = {
        ...
        (
            ...
            (vec[9 * vec.len() / 10] - vec[vec.len() / 10]) as f64,
        )
    };
...
impl<D: Default + Clone> StatTimer<D> {
    pub(crate) fn new(warm_up: usize, num: usize) -> Self {
        Self {
            warm_up,
            num,
            record: Vec::with_capacity(num + warm_up),
            ..Default::default()
        }
    }
    ...
    pub(crate) fn calculate_stats(&self) -> (DescStats, DescStats) {
        let iter1 = self
            ...
            .iter()
            .skip(self.warm_up)
            .map(|r| r.duration.as_micros());
        let iter2 = self
            .record
            .iter()
            .skip(self.warm_up)
            .map(|r| r.full_duration.as_micros());
        (
            calculate_stats_from(self.num, iter1),
            calculate_stats_from(self.num, iter2),
        )
    }
}
